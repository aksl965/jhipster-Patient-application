...
public class AdjudicationResource {
    ...
    private static final String ENTITY_NAME = "jhipsterSamplePatientAdjudication";
    ...
    public ResponseEntity<List<Adjudication>> getAllAdjudications(Pageable pageable, @RequestParam MultiValueMap<String, String> queryParams, UriComponentsBuilder uriBuilder, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Adjudications");
        Page<Adjudication> page;
        if (eagerload) {
            page = adjudicationRepository.findAllWithEagerRelationships(pageable);
        } else {
            page = adjudicationRepository.findAll(pageable);
        }
    ...
    public ResponseEntity<Adjudication> getAdjudication(@PathVariable Long id) {
        ...
        Optional<Adjudication> adjudication = adjudicationRepository.findOneWithEagerRelationships(id);
