...
import io.github.jhipster.application.JhipsterSamplePatientApp;
 ...
 */
@SpringBootTest(classes = JhipsterSamplePatientApp.class)
public class AdjudicationResourceIT {
    ...
    @Mock
    private AdjudicationRepository adjudicationRepositoryMock;
    ...
    public void getAllAdjudicationsWithEagerRelationshipsIsEnabled() throws Exception {
        AdjudicationResource adjudicationResource = new AdjudicationResource(adjudicationRepositoryMock);
        when(adjudicationRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        MockMvc restAdjudicationMockMvc = MockMvcBuilders.standaloneSetup(adjudicationResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
            .setMessageConverters(jacksonMessageConverter).build();

        restAdjudicationMockMvc.perform(get("/api/adjudications?eagerload=true"))
        .andExpect(status().isOk());

        verify(adjudicationRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({"unchecked"})
    public void getAllAdjudicationsWithEagerRelationshipsIsNotEnabled() throws Exception {
        AdjudicationResource adjudicationResource = new AdjudicationResource(adjudicationRepositoryMock);
            when(adjudicationRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
            MockMvc restAdjudicationMockMvc = MockMvcBuilders.standaloneSetup(adjudicationResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
